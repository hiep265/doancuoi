...
export class SanPhamBienTheComponent implements OnInit {
  ...
  onSubmit = (data) => {
    console.log('Current ID:', this.service.sanphambienthe.id);
    
    if (this.service.sanphambienthe.id == 0 || !this.service.sanphambienthe.id) {
      ...
      formData.append('SizeId', data.Id_Size);
      formData.append('SoLuongTon', data.SoLuongTon);
      const id = this.service.sanphambienthe.id || 0;
      console.log('Update ID:', id);
      if (id !== 0) {
        this.http.put(environment.URL_API + 'sanphambienthes/' + id, formData)
          .subscribe(res => {
        ...
        console.error('Cannot update with ID=0 or undefined');
        this.serviceToast.showToastSuaThatBai();
      }
    }
  }
