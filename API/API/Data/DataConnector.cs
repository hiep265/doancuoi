...
namespace API.Data
{
    ...
    {
        Task<List<TenSPSoLanXuatHienTrongDonHang>> GetSoLanXuatHienTrongDonHang(int? year = null);
        Task<List<TenSanPhamDoanhSo>> Top10SanPhamLoiNhats(int? year = null);
        Task<List<NhanHieuBanChayNhatTrongNam2021>> GetNhanHieuBanChayNhatTrongNam(int? year = null);
        Task<List<DataSetBanRaTonKho>> DataDataSetBanRaTonKho(int? year = null);
        Task<List<NhaCungCapSoLuong>> GetNhaCungCapSoLuongs();
        Task<List<NhaCungCapTongTien>> GetDoanhSoBans();
        Task<Nam2021SoTongTien> GetNamTongTien(int? year = null);
    ...
    public class DataConnector: IDataConnector
    {
        ...
        public async Task<List<TenSPSoLanXuatHienTrongDonHang>> GetSoLanXuatHienTrongDonHang(int? year = null)
        {
            ...
            var sql = $@"select top(10) SanPhams.Ten+' '+Sizes.TenSize+' '+MauSacs.MaMau as 'Ten', sum(ChiTietHoaDons.Soluong) as 'Số lần tồn tại trong đơn hàng'
					    ...
					    on ChiTietHoaDons.Id_HoaDon = HoaDons.Id
                         where HoaDons.TrangThai = 2 AND DATEPART(YYYY, HoaDons.NgayTao) = '{selectedYear}'
        ...
        public async Task<List<TenSanPhamDoanhSo>> Top10SanPhamLoiNhats(int? year = null)
        {
            int selectedYear = year ?? DateTime.Now.Year;
        ...
        public async Task<List<NhanHieuBanChayNhatTrongNam2021>> GetNhanHieuBanChayNhatTrongNam(int? year = null)
        {
            int selectedYear = year ?? DateTime.Now.Year;
            ...
            string sql = $@"select top(10) NhanHieus.Ten,sum(ChiTietHoaDons.Soluong) as'soluong'
                            ...
                            where DATEPART(YYYY,HoaDons.NgayTao)='{selectedYear}' and HoaDons.TrangThai = 2
        ...
        public async Task<List<DataSetBanRaTonKho>> DataDataSetBanRaTonKho(int? year = null)
        {
            int selectedYear = year ?? DateTime.Now.Year;
...
";
        ...
        public async Task<Nam2021SoTongTien> GetNamTongTien(int? year = null)
        {
            int selectedYear = year ?? DateTime.Now.Year;
            ...
            if (reader.HasRows)
            {
                while (await reader.ReadAsync())
                {
                    nam.Nam = (int)reader["Nam"];
                    nam.TongTien = (decimal)reader["Tong tien trong nam"];
                }
            }
            else
            {
                nam.Nam = selectedYear;
                nam.TongTien = 0;
            }
            cnn.Close();
            return nam;
