...
$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
  ...
  addToCard(product) {
    console.log("addToCard được gọi");
    console.log("Màu:", this.selectMau);
    console.log("Size:", this.selectSize);
    console.log("Số lượng:", this.soLuong);
    ...
    // Kiểm tra từng điều kiện riêng biệt để hiển thị thông báo cụ thể
    if(this.selectMau === null || this.selectMau === undefined || this.selectMau === 'null') {
      Swal.fire({
        icon: 'warning',
    ...
    if(this.selectSize === null || this.selectSize === undefined || this.selectSize === 'null') {
      Swal.fire({
        icon: 'warning',
    ...
    try {
      // Nếu tất cả điều kiện hợp lệ, tiến hành thêm vào giỏ hàng
      ...
      if (!SanPhamBienThe) {
        Swal.fire({
          icon: 'error',
          title: 'Lỗi',
          text: 'Không tìm thấy sản phẩm với màu sắc và kích thước đã chọn!',
        });
        return;
      }
